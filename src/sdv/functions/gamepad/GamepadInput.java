...
public class GamepadInput {
    ...
    /**
     * Same as GamepadInput(dll, 1, 50, 50).
    ...
    /**
     * Same as GamepadInput(dll, 1, controllerPollPeriod, queuePollPeriod).
     * The DLL xboxcontroller.dll or xboxcontroller64.dll is searched in
     * the Windows path (depending on the OS version).
     */
    public GamepadInput(int controllerPollPeriod, int queuePollPeriod) {
        if (is64bit())
    ...
    /**
     * Same as GamepadInput(dll, int playerNb, 50, 50).
     * The DLL xboxcontroller.dll or xboxcontroller64.dll is searched in
     * the Windows path (depending on the OS version).
     */
    public GamepadInput(int playerNb) {
        if (is64bit())
    ...
    /**
     * Creates a GamepadInput instance using the given native Windows DLL
     ...
     */
    public GamepadInput(String dll, int playerNb, int controllerPollPeriod, int queuePollPeriod) {
    ...
    private void init(String dll, int playerNb, int controllerPollPeriod, int queuePollPeriod) {
        System.out.println("GamepadInput V" + version + " (www.aplu.ch)");
        ...
        if (playerNb < 1 || playerNb > 4) {
            JOptionPane.showMessageDialog(null, "GamepadInput number must be 1..4",
                    "Fatal error while creating GamepadInput", JOptionPane.ERROR_MESSAGE);
            System.exit(1);
        }
    ...
    public synchronized void vibrate(int leftVal, int rightVal, int duration) {
        ...
        if (leftVal < 0 || leftVal > 65535) {
            JOptionPane.showMessageDialog(null, "left value must be 0..65535",
                    "Fatal error while calling GamepadInput.vibrate", JOptionPane.ERROR_MESSAGE);
            System.exit(1);
        }
    ...
    /**
     * Registers the given GamepadListener and starts polling the event queue.
     ...
     *
     * @param gamepadListener the GamepadListener to register
     */
...
//          cs.write();
                    if ((cs.source & ControllerState.SOURCE_BUTTONS) != 0) {
                        ...
                        for (int i = 8; i < 18; i++) {
                            ...
                            } else if ((buttonState & bitValue) != 0) {
